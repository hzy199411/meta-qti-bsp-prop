...
def get_depends(d):
    ...
    elif d.getVar('DISTRO', True) == 'robot-som':
        return "${MLPREFIX}rb-mm-camera"
    elif d.getVar('DISTRO', True) == 'robot-som-ros':
        return "${MLPREFIX}rb-mm-camera"
    elif d.getVar('MACHINE', True) == 'sda845':
        return "camx av-frameworks binder libhardware virtual/kernel"
    elif d.getVar('PRODUCT', True) == 'qsap':
        return ""
    else:
        return "${MLPREFIX}mm-camera"

DEPENDS += "glib-2.0 diag libxml2 libcutils liblog"
...
LDFLAGS_append_robot-som += "-llog"
LDFLAGS_append_robot-som-ros += "-llog"
...
do_install_append() {
    ...
    #Add start script
    install -m 0644 ${WORKDIR}/ffbm_start.sh ${D}/usr/bin
    if ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'true', 'false', d)}; then
        install -d ${D}${systemd_unitdir}/system/
        install -m 0644 ${WORKDIR}/ffbm_mmi.service -D ${D}${systemd_unitdir}/system/ffbm_mmi.service
        install -m 0644 ${WORKDIR}/ffbm_start.service -D ${D}${systemd_unitdir}/system/ffbm_start.service
