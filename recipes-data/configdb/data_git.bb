...
pkg_postinst_${PN}_mdm () {
        ...
        if [ "${MACHINE}" != "mdm9607" ]; then
          ...
          update-rc.d $OPT start_ipacmdiag_le start 32 S . stop 62 0 1 6 .
        fi
        ...
        if [ ${@d.getVar('PRODUCT', True)} != "psm" ]; then
           ...
           update-rc.d $OPT -f start_qti_le remove
           update-rc.d $OPT start_qti_le start 32 S . stop 60 0 1 6 .
           update-rc.d $OPT -f start_adpl_le remove
...
do_install_append_msm() {
        ...
        if ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'true', 'false', d)}; then
          ...
          #QCMAP service
          if (test "${BASEMACHINE}" != "sda845") && (test "x${BASEPRODUCT}" != "xdrone"); then
