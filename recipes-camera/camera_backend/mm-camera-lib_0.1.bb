...
DEPENDS += "glib-2.0 libxml2 cameradbg fastcv-noship"
...
do_install_append(){
    ...
    bayer_proc_binary="shdr-gpu-pass-fusion-and-tm-fast.cl"
    ...
    bayer_proc_binary="shdr-gpu-pass-fusion-and-tm.cl"
    if [ -f "$bayer_proc_binary_location/$bayer_proc_binary" ];then
        cp $bayer_proc_binary_location/$bayer_proc_binary $binaries_out
    fi
    ...
    cp ${S}/bayer_proc/prebuilt/svhdr/lib/libShdr_LE.so ${D}/usr/lib/libShdr_LE.so
}
